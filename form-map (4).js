 ...
 */
const PIPE_OPTIONS = ["N/A", '1-1/4"', '1-1/2"', '2"'];
...
module.exports = {
  fill(d, input) {
    ...
    if (input.model) d.set('model', input.model);
  ...
  formChecks(d, check) {
    ...
    check('no min-flow field (not on this tool)', doc.getElementById('usg-minflow') === null);
    const modelOpts = Array.from(doc.querySelectorAll('#usg-model option')).map(o => o.textContent);
    check('model selector offers every variant',
      JSON.stringify(modelOpts) === JSON.stringify(
        ['N/A (any)', '243-RPC', '243-RPC-A', '243-RPC-B']), modelOpts);
    ...
    check('pipe options are the RPC set',
